...
use zot_core::{AppConfig, DesktopBridgeConfig, ZotError};
use zot_desktop::{BRIDGE_PROTOCOL_VERSION, BridgeHealth, BridgeStatus};
...
async fn handle_revoke(ctx: &AppContext, args: BridgeRevokeArgs) -> Result<CommandOutput> {
    ...
    if !args.local_only {
        ...
        ctx.desktop()?.revoke(&bridge.token).await?;
    }
