...
pub(crate) async fn handle(ctx: &AppContext, command: ItemCommand) -> Result<CommandOutput> {
    match command {
        ...
        ItemCommand::Create(args) => legacy(write::handle_create(ctx, args)).await,
        ItemCommand::AddDoi(args) => legacy(write::handle_add_doi(ctx, args)).await,
        ItemCommand::AddUrl(args) => legacy(write::handle_add_url(ctx, args)).await,
        ItemCommand::AddFile(args) => legacy(write::handle_add_file(ctx, args)).await,
        ItemCommand::Merge(args) => legacy(write::handle_merge(ctx, args)).await,
        ItemCommand::Update(args) => legacy(write::handle_update(ctx, args)).await,
        ItemCommand::Trash(args) => legacy(write::handle_trash(ctx, args)).await,
        ItemCommand::Restore(args) => legacy(write::handle_restore(ctx, args)).await,
        ItemCommand::Attach(args) => legacy(write::handle_attach(ctx, args)).await,
        ItemCommand::Note { command } => legacy(note::handle(ctx, command)).await,
