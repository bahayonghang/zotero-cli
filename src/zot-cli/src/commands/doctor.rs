...
const DOCTOR_BANNER: &str = r#"       .-----------------------.
    ...
    |  |      `-'-'          |  |
     \  \   zot doctor      /  /
      \  '-----------------'  /
       '---------------------'"#;
const LEGACY_BRIDGE_HINT: &str = "Zot Bridge has been removed: delete the desktop_bridge and write_backend entries from the config file, then uninstall Zot Bridge from Zotero's plugin manager.";
...
pub(crate) async fn handle(ctx: &AppContext) -> Result<CommandOutput> {
    ...
    let payload = serde_json::json!({
        ...
        "migration_hints": migration_hint.into_iter().collect::<Vec<_>>(),
        "capabilities": {
...
fn legacy_bridge_config_present(path: &std::path::Path) -> bool {
    ...
    let has_legacy_keys = |table: &toml::map::Map<String, toml::Value>| {
        table.contains_key("desktop_bridge") || table.contains_key("write_backend")
    };
    let root_has_legacy = value
        .get("zotero")
        .and_then(toml::Value::as_table)
        .is_some_and(has_legacy_keys);
    let profile_has_legacy = value
        .get("profile")
        .and_then(toml::Value::as_table)
    ...
    root_has_legacy || profile_has_legacy
}
...
mod tests {
    ...
    #[test]
    fn legacy_bridge_config_detection_covers_root_and_profiles_without_exposing_values() {
        for raw in [
            "[zotero]\nwrite_backend = 'desktop'\n",
            "[zotero.desktop_bridge]\ntoken = 'root-secret'\n",
            "[profile.work]\nwrite_backend = 'desktop'\n",
            "[profile.work.desktop_bridge]\ntoken = 'profile-secret'\n",
        ] {
            let dir = tempfile::tempdir().expect("tempdir");
            let path = dir.path().join("config.toml");
            std::fs::write(&path, raw).expect("write fixture");
            assert!(legacy_bridge_config_present(&path), "fixture: {raw}");
        }

        assert!(!LEGACY_BRIDGE_HINT.contains("root-secret"));
        assert!(!LEGACY_BRIDGE_HINT.contains("profile-secret"));
    }
